		...
		<div class="container">
			...
			<div style="width: 40%; margin: 20px auto;">
				<form method="post" action="register.php" onsubmit="return userCheck()" id = "usercheck" >
					...
					<br>
					<p>Password must contain at least one number, one uppercase, and one lowercase letter.  It must also be at least 8 or more characters.  Do not use a password from elsewhere.</p>
					<br>
					...
					if ($num_results == 1){
						...
						if($tool == 1){
							if($user_mail != ""){   
								...
								$Query = "INSERT INTO Users (Username, Email, Password, Image, TwScrape) VALUES ('$name_user','$user_mail', '$hashed_password', '$arr[$rand_img]', '1')";
								$Result = mysqli_query($conn,$Query); 	
							...
							if($user_mail != ""){   
								...
								$Query = "INSERT INTO Users (Username, Email, Password, Image, nmap) VALUES ('$name_user','$user_mail', '$hashed_password', '$arr[$rand_img]', '2')";
								$Result = mysqli_query($conn,$Query);
								if ($Result == true){           
									header("Location:index.php");
								} else if($Result == false){
